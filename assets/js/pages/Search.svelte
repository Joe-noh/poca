...
<div class="p-2 sm:p-3 sticky top-0 left-0 right-0 border-b border-hairline bg-paper">
  <form class="group flex flex-row items-center gap-2 bg-paper-hover px-2 rounded-md" onsubmit={handleSubmit}>
    ...
    <input bind:value={query} type="search" inputmode="search" class="w-full py-2 outline-none font-serif text-base" />
  </form>
</div>
{#if isLoading}
  <span>Searching...</span>
{:else}
  <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-x-2 gap-y-4 p-2 sm:gap-6 sm:p-6 min-h-screen">
