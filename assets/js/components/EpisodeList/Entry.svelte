...
<button
  type="button"
  class="flex flex-row text-left items-start gap-3 p-1.5 w-full rounded-sm cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-ink transition-colors"
  onclick={handleClick}
>
  ...
  <div class="flex flex-col justify-between gap-1 w-full min-h-12 h-full">
    <span class="text-base font-serif text-ink">{episode.title}</span>
    <div class="flex flex-row justify-between items-end">
      <span class="text-sm text-muted font-serif">{publishedAt}</span>
